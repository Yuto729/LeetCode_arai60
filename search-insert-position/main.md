...
- 重複要素がある場合どう書く？ -> 最左のインデックスを返すと想定する。early returnなしで書く。early returnしてしまうと、最初にtargetに当たった時点で返してしまうので最左端が保証されない。
- 再帰で実装するパターン
...
## Step3
early returnなし・半開区間
- leftより左は全てtarget未満、right以降は全部target以上なので、left == rightで止まったらその位置が**targetに等しい、またはそれより大きい最初の位置**になる。
