...
```
A: movl (%rdx), %ecx    # prices[i] 読み込み
B: movl %ecx,  %edi     # A に依存
C: subl %eax,  %edi     # B と eax(min_price) に依存
D: cmpl %edi,  %esi     # C と esi(max_profit) に依存
E: cmovl %edi, %esi     # D のフラグに依存  → max_profit 更新
F: cmpl %ecx,  %eax     # A と eax(min_price) に依存
G: cmovg %ecx, %eax     # F のフラグに依存  → min_price 更新
H: addq $4,    %rdx     # 他と独立
I: cmpq %r8,   %rdx     # H に依存
J: jne .L4              # I のフラグに依存
```
...
**並列化できるペア：**
- **D と F**：Dは`edi`、Fは`ecx`と`eax`を参照。互いに独立なので同時発行できる
- **H**：ポインタのインクリメントで完全独立。どこでも並列実行できる

**直列になる依存チェーン：**
```
A → B → C → D → E  （max_profit更新）
A → F → G           （min_price更新）
H → I → J           （ループカウンタ）
```

AのL1キャッシュアクセスが実質的なボトルネック。L1ヒットなら4〜5クロック程度で、後続の演算はその間に並列処理される。
...
## 用語集 (computer architecture復習用)

| 用語 | 説明 |
|------|------|
| **IPC** (Instructions Per Cycle) | 1クロックあたりの実行命令数。IPC>1はスーパースカラが効いている証拠 |
| **スーパースカラ** | 1クロックに複数の命令を並列発行できるCPUアーキテクチャ。現代x86は3〜5命令/クロック |
| **パイプライン** | 命令をIF(instruction fetch)→ID(instruction decode)→EX(execute)→MEM(memory access)→WB(write back)の段階に分けて重ねて実行する仕組み |
| **パイプラインフラッシュ** | 分岐予測ミスなどで先読みした命令を捨てて最初からやり直すこと。数十クロックのペナルティ |
| **フォワーディング** | 前の命令の結果をWBを待たず次の命令のEXに直接転送する回路。多くのRAWハザードを解消する |
| **アウトオブオーダー実行** | 依存関係のない命令を順序に関わらず先行実行すること。ストール中の無駄を減らす |
| **データハザード** | パイプラインで命令間のデータ依存が原因で発生するストール |
| **RAW** (Read After Write) | 前の命令が書いた値を後の命令が読む「真の依存」。解消不可 |
| **WAR** (Write After Read) | 前の命令が読む前に後の命令が書く「偽の依存」。アウトオブオーダーで発生し得る |
| **WAW** (Write After Write) | 同じレジスタに2回書く「偽の依存」。アウトオブオーダーで発生し得る |
| **ロードユースハザード** | ロード命令の直後にその値を使う命令が来るRAWハザード。フォワーディングでも1クロックストールが残る |
| **CMOVcc** | x86の条件付き転送命令。条件分岐（JNE等）と異なりジャンプしないためパイプラインが止まらない |
| **CSEL** | ARMにおけるCMOVccの対応命令 |
| **分岐予測** | CPUが分岐先を実行前に予測して先読みする仕組み。予測ミスでフラッシュが発生する |
| **L1キャッシュ** | CPU内の最も近いキャッシュ。アクセスに4〜5クロック程度かかる |